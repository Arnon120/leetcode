...
impl Solution {
    ...
    pub fn merge(nums1: &mut Vec<i32>, m: i32, nums2: &mut Vec<i32>, n: i32) { 
        ...
        while k > 0 {
            ...
            if get_from_array_with_distorted_index(nums1, i) < get_from_array_with_distorted_index(nums2, j) {
                nums1[k-1] = nums2[j-1];
                j -= 1;
            } 
            else {
...
mod test {
    ...
    fn test_merge(#[case] mut nums1: Vec<i32>, #[case] mut nums2: Vec<i32>, #[case] expected: Vec<i32>) {
        let m = nums1.len();
        let n = nums2.len();
